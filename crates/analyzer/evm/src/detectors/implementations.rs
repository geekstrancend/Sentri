...
pub fn detect_missing_signer_check(source: &str, file_path: &str) -> Vec<Finding> {
    ...
    for (line_num, line) in source.lines().enumerate() {
            ...
            && !trimmed.contains("view")
        {
            ...
            let has_access_check = source.lines()
                .skip(line_num)
                .take(20)
                .any(|l| {
            ...
            let modifies_state = source.lines()
                .skip(line_num)
                .take(20)
                .any(|l| {
                    let lower = l.to_lowercase();
                    (lower.contains(" = ") || lower.contains("transfer") || lower.contains("mint") || lower.contains("burn"))
                        && !l.trim().starts_with("//")
                });
    ...
    for (line_num, line) in source.lines().enumerate() {
        ...
        if (lower.contains("function swap") || lower.contains("function exchange"))
            ...
            let has_k_check = func_body.to_lowercase().contains("require(")
                && (func_body.to_lowercase().contains("* ") || func_body.to_lowercase().contains("*"));
            ...
            if modifies_reserves && !has_k_check {
                findings.push(
                    Finding::new(
                        ...
                        line_num + 1,
                        0,
                        "AMM swap function does not verify x*y=k invariant after exchange".to_string(),
                        line.trim().to_string(),
                    )
...
pub fn detect_oracle_spot_price(source: &str, file_path: &str) -> Vec<Finding> {
    ...
    for (line_num, line) in source.lines().enumerate() {
        ...
        if (lower.contains("balanceof") || lower.contains("reserve") || lower.contains("getreserves"))
            && (lower.contains("price") || lower.contains("rate") || lower.contains("amount") || lower.contains("="))
                    ...
                    if (lower.contains(" + ") || lower.contains(" - ") || lower.contains(" * "))
                        ...
                        findings.push(
                            Finding::new(
                                ...
                                line_num + 1,
                                0,
                                "Arithmetic operation in Solidity <0.8.0 without SafeMath".to_string(),
                                line.trim().to_string(),
                            )
...
pub fn detect_flash_loan_governance(source: &str, file_path: &str) -> Vec<Finding> {
    ...
    for (line_num, line) in source.lines().enumerate() {
        ...
        if lower.contains("block.number") 
            && (lower.contains("vote") || lower.contains("snapshot") || lower.contains("vote_power")) {
...
pub fn detect_reentrancy_erc20(source: &str, file_path: &str) -> Vec<Finding> {
    ...
    for (line_num, line) in source.lines().enumerate() {
        ...
        if (lower.contains("erc20") || lower.contains("ierc20")) 
            ...
            let following = source.lines()
                .skip(line_num)
                .take(10)
                .any(|l| l.to_lowercase().contains("balances[") || l.to_lowercase().contains("_balances["));
        ...
        if (line.contains("function") && line.contains("public") || line.contains("external"))
            ...
            findings.push(
                Finding::new(
                    ...
                    line_num + 1,
                    0,
                    "Permissionless relay function allows anyone to execute arbitrary transactions".to_string(),
                    line.trim().to_string(),
                )
...
pub fn detect_single_eoa_admin(source: &str, file_path: &str) -> Vec<Finding> {
    ...
    for (line_num, line) in source.lines().enumerate() {
        if (line.contains("owner") || line.contains("admin")) 
            ...
            let has_contract_check = source.contains("isContract") || source.contains(".code.length");
